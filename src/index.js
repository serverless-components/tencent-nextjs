...
class ServerlessComponent extends core_1.Component {
    getCredentials() {
        ...
        if (!tmpSecrets || !tmpSecrets.TmpSecretId) {
            throw new error_1.ApiTypeError('CREDENTIAL', 'Cannot get secretId/Key, your account could be sub-account and does not have the access to use SLS_QcsRole, please make sure the role exists first, then visit https://cloud.tencent.com/document/product/1154/43006, follow the instructions to bind the role to your account.');
        }
        return {
    ...
    async uploadCodeToCos(appId, inputs, region) {
        ...
        const bucketName = ((_a = inputs.code) === null || _a === void 0 ? void 0 : _a.bucket) || `sls-cloudfunction-${region}-code`;
        const objectName = ((_b = inputs.code) === null || _b === void 0 ? void 0 : _b.object) || `${inputs.name}-${Math.floor(Date.now() / 1000)}.zip`;
        // if set bucket and object not pack code
        if (!((_c = inputs.code) === null || _c === void 0 ? void 0 : _c.bucket) || !((_d = inputs.code) === null || _d === void 0 ? void 0 : _d.object)) {
    ...
    async deployFunction(credentials, inputs = {}, region) {
        ...
        outputs.lastVersion = (_b = (_a = scfOutput.LastVersion) !== null && _a !== void 0 ? _a : this.state.lastVersion) !== null && _b !== void 0 ? _b : '$LATEST';
        // default traffic is 1.0, it can also be 0, so we should compare to undefined
        outputs.traffic = (_d = (_c = scfOutput.Traffic) !== null && _c !== void 0 ? _c : this.state.traffic) !== null && _d !== void 0 ? _d : 1;
    ...
    async deployApigw(credentials, inputs, region) {
        var _a, _b;
        const { state } = this;
        const serviceId = (_a = inputs.serviceId) !== null && _a !== void 0 ? _a : (state && state.serviceId);
        const apigw = new tencent_component_toolkit_1.Apigw(credentials, region);
        const oldState = (_b = this.state) !== null && _b !== void 0 ? _b : {};
        const apigwInputs = Object.assign(Object.assign({}, inputs), { oldState: {
        ...
        const outputs = {
            ...
            url: `${utils_1.getDefaultProtocol(inputs.protocols)}://${apigwOutput.subDomain}/${apigwOutput.environment}${apigwInputs.endpoints[0].path}`,
        };
        if (apigwOutput.customDomains) {
            outputs.customDomains = apigwOutput.customDomains;
        }
        this.state = Object.assign(Object.assign(Object.assign({}, this.state), outputs), { apiList: apigwOutput.apiList, created: true });
        return outputs;
    }
    ...
    async deployStatic(inputs, region) {
        ...
        if (zipPath) {
            ...
            // 1. deploy to cos
            const { staticCosInputs, bucket, policy } = await formatter_1.formatStaticCosInputs(inputs.cosConf, appId, zipPath, region);
            const cos = new tencent_component_toolkit_1.Cos(credentials, region);
            const cosOutput = {
                region,
                bucket,
            ...
            if (inputs.cdnConf) {
                ...
                const cdnInputs = await formatter_1.formatStaticCdnInputs(inputs.cdnConf, cosOutput.cosOrigin);
    ...
    async deploy(inputs) {
        ...
        const credentials = this.getCredentials();
        // 对Inputs内容进行标准化
        const { region, functionConf, apigatewayConf } = await formatter_1.formatInputs(this.state, inputs);
        // 部署函数 + API网关
        const outputs = {};
    ...
    async remove() {
        ...
        const { state } = this;
        const { region } = state;
        const { namespace, functionName, created, serviceId, apigwDisabled, customDomains, apiList, environment, } = state;
        const credentials = this.getCredentials();
    ...
    async metrics(inputs = {}) {
        ...
        if (!inputs.rangeStart || !inputs.rangeEnd) {
            throw new error_1.ApiTypeError(`PARAMETER_${CONFIGS.framework.toUpperCase()}_METRICS`, 'rangeStart and rangeEnd are require inputs');
        }
        const { state } = this;
        const { region } = state;
        if (!region) {
            throw new error_1.ApiTypeError(`PARAMETER_${CONFIGS.framework.toUpperCase()}_METRICS`, 'No region property in state');
        }
        ...
        if (functionName) {
            ...
            const metricResults = await mertics.getDatas(inputs.rangeStart, inputs.rangeEnd, tencent_component_toolkit_1.Metrics.Type.All);
            return metricResults;
        }
        throw new error_1.ApiTypeError(`PARAMETER_${CONFIGS.framework.toUpperCase()}_METRICS`, 'Function name not define');
    }
}

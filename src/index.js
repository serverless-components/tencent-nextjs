...
class TencentEgg extends Component {
  ...
  async prepareInputs(inputs = {}) {
    inputs.name =
      ...
      `NextjsComponent_${random({ length: 6 })}`
  ...
  async default(inputs = {}) {
    ...
    inputs.fromClientRemark = inputs.fromClientRemark || 'tencent-nextjs'
    const tencentCloudFunctionOutputs = await tencentCloudFunction(inputs)
    const apigwParam = {
      serviceName: inputs.serviceName,
      description: 'Serverless Framework tencent-nextjs Component',
    ...
    apigwParam.fromClientRemark = inputs.fromClientRemark || 'tencent-nextjs'
  ...
  async remove(inputs = {}) {
    const removeInput = {
      fromClientRemark: inputs.fromClientRemark || 'tencent-nextjs'
    }
    const tencentCloudFunction = await this.load('@serverless/tencent-scf')
    const tencentApiGateway = await this.load('@serverless/tencent-apigateway')
